 ...
 */
public class ModalVistaAsignarDireccionesController implements Initializable {
    ...
    public void closeApp(SucursalesController controllersucursal) {
        sucursalController = controllersucursal;
    }
    ...
    private void txtnumerosegundario(KeyEvent event) {
        char c = event.getCharacter().charAt(0);
        if (!Character.isDigit(c) || txtnumerosegundario.getText().length() > 3) {
            event.consume();
        }
    }
     @FXML
    ...
    private void btnenviar(ActionEvent event) {
        ...
        if (txtvias.getText().equals("") || numeroprincipal.getText().equals("") || txtbarrio_localida.getText().equals("")) {
            ...
            sucursalController.asignardireccion(direccion);
        }
