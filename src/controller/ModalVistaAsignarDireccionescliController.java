 ...
 */
public class ModalVistaAsignarDireccionescliController implements Initializable {
    ...
    public void closeApp(GestionCuentasController controllergetionCuentas) {
        getioncuentasController = controllergetionCuentas;
    }
  ...
  @FXML
    private void txtnumerosegundario1(KeyEvent event) {
        char c = event.getCharacter().charAt(0);
        if (!Character.isDigit(c) || txtnumerosegundario1.getText().length() > 3) {
            event.consume();
        }
    }
    @FXML
    ...
    private void bodegaaction(ActionEvent event) {
        labeltipovivineda.setText("Bodega");
    }

    private void localaction(ActionEvent event) {
        labeltipovivineda.setText("Local");
    }
    @FXML
    private void btnenviar(ActionEvent event) {
        ...
        if (txtvias.getText().equals("") || numeroprincipal.getText().equals("") || txtbarrio_localida.getText().equals("")) {
           ...
           getioncuentasController.asignardireccion(direccion);
        }
